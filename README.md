...
- `interactions/`: JSON-based interaction scripts that replay deterministic user gestures (typing, clicks, custom JS) on top of the recordings.  
  You can override the runtime path (`/root/interactions/`) via the `ARCANUM_INTERACTIONS_DIR` environment variable so the harness looks in the same folder you copied into the container.
...
**Note**: Tests now support automatic results separation for comparing passive vs. interaction-enabled runs. See `Test_Cases/INTERACTION_RESULTS.md` for details on configuring environment variables and analyzing the structured logs.
...
Interaction-driven scenarios: the harness now loads optional JSON specifications from `/root/interactions/`. These files describe deterministic user gestures (typing, clicking, custom script execution) that run immediately after the replayed page loads. Tests such as `Custom_Interaction_Test` rely on these scripts to trigger leaks that only occur after user input. See `interactions/README.md` for the schema and authoring tips. When no interaction file exists, the harness skips this phase and behaves exactly as before.
...
* Optional interaction scripts live in `interactions/` (documented in `interactions/README.md`). Copy them to `/root/interactions/` to enable interaction-driven leakage measurements.
